#!/usr/bin/env python3
"""Run the frozen v14 48-cell scorer batch after exact external authorization."""
import argparse
import asyncio
import fcntl
import hashlib
...
INPUT = Path("research/experiments/2026-09-14-g5-fresh-family-v14-scorer-preflight/inputs.json")
OUTPUT = Path("research/experiments/2026-09-14-g5-fresh-family-v14-scorer-predictions")
EXPECTED_INPUT_SHA = "3c6a948f396fa7d68426df10b506f342188eccf8b57088701c7e841e11744f48"
AUTHORIZATION_SCHEMA = "g5-fresh-v14-scorer-external-authorization-v1"
AUTHORIZATION_SCOPE = (
    "Send the 48 frozen dimension-scoped development-scoring requests derived from the eight "
    "sealed v14 dialogues to https://ollama.com/api/chat using gpt-oss:120b; preserve every raw "
...
def validate_inputs():
    ...
    if (bundle.get("sha256") != EXPECTED_INPUT_SHA
            ...
            != EXPECTED_INPUT_SHA):
        raise ValueError("Frozen v14 scorer inputs changed")
        ...
        for case in bundle["cases"]:
            for number in (1, 2):
                ...
                if result["status"] == "completed":
                    break
        print("FROZEN_FRESH_V14_SCORER_PREDICTIONS_FINISHED", flush=True)
    finally:
        os.close(lock)
