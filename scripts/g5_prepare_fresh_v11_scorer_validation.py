#!/usr/bin/env python3
"""Prepare the sealed v11 development dialogues for blinded review and scoring.
...
SOURCE = Path("research/experiments/2026-09-13-g5-fresh-family-v11-development-online")
TARGET = Path("research/experiments/2026-09-13-g5-fresh-family-v11-scorer-preflight")
EXECUTION_SHA = "1f6b4e7b030bc608df66c3274f6adbfc7e3d9c9abc0967912840968d7dde4fd4"
TRANSCRIPTS_SHA = "03f9ebf1d1a701a8f42199051ae9758b4804dda54edc5c97536d4fc37bff5a48"
...
def main():
    ...
    binding = ModelBinding("ollama", "gpt-oss:120b",
        "ollama-cloud-g5-fresh-scorer-v1", 0, 8192)
    predictor = CatalogEvidencePredictor(binding,
        OllamaTransport(binding, "https://ollama.com", timeout=240),
        predictor_id="g5-fresh-v11-development-scorer-20260913", kind="ai")
    plan = predictor.plan()
    cases, source_files = [], {}
    for assignment in sorted(execution["selected_assignments"], key=lambda item: item["scenario_id"]):
        ...
        for dimension, rubric in RUBRICS.items():
            task = {"case_id": f"fresh-v11-{scenario_id}-{dimension}",
    ...
    bundle = {"schema": "g5-fresh-v11-scorer-inputs-v1",
        ...
        "selection": "All eight sealed v11 development dialogues and all six dimensions; no score-based filtering.",
    ...
    distribution = {"schema": "g5-fresh-v11-reference-distribution-plan-v1",
    ...
    summary = {"schema": "g5-fresh-v11-scorer-preflight-summary-v1",
