#!/usr/bin/env python3
"""Run the frozen v17 48-cell scorer batch after exact external authorization."""
import argparse
import asyncio
import fcntl
import hashlib
...
INPUT = Path("research/experiments/2026-09-14-g5-fresh-family-v17-scorer-preflight/inputs.json")
OUTPUT = Path("research/experiments/2026-09-15-g5-fresh-family-v17-scorer-predictions")
EXPECTED_INPUT_SHA = "ea35300e0680a1146f0daf998a3d28ebfe786cdf587cd6e65430b97e589c68ea"
AUTHORIZATION_SCHEMA = "g5-fresh-v17-scorer-external-authorization-v1"
AUTHORIZATION_SCOPE = (
    "Send the 48 frozen dimension-scoped development-scoring requests derived from the eight "
    "sealed v17 dialogues to https://ollama.com/api/chat using gpt-oss:120b; preserve every raw "
...
def validate_inputs():
    ...
    if (bundle.get("sha256") != EXPECTED_INPUT_SHA
            ...
            != EXPECTED_INPUT_SHA):
        raise ValueError("Frozen v17 scorer inputs changed")
        ...
        for case in bundle["cases"]:
            for number in (1, 2):
                ...
                if result["status"] == "completed":
                    break
        print("FROZEN_FRESH_V17_SCORER_PREDICTIONS_FINISHED", flush=True)
    finally:
        os.close(lock)
