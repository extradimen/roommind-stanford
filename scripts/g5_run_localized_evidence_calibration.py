"""Run frozen v3 localized evidence calibration; credential stays in memory."""
import asyncio
import fcntl
import hashlib
import json
...
async def attempt(case,plan,number,key,http_transport=None):
    ...
    if paths[0].exists():
        if read(paths[0])!=marker: raise ValueError('Attempt drift')
    else: save(paths[0],marker)
    if paths[2].exists(): return read(paths[2])
    ...
    predictor=LocalizedEvidencePredictor(binding,OllamaTransport(binding,'https://ollama.com',timeout=240,
        http_transport=httpx.MockTransport(replay)),predictor_id=plan['predictor']['id'],kind='ai')
    try:
    ...
    save(paths[2],result); return result


async def run():
    bundle=read(INPUT)
    if bundle['sha256']!=digest({k:v for k,v in bundle.items() if k!='sha256'}) or bundle['external_execution_authorized'] is not False:
        raise ValueError('Frozen v3 input or authorization marker changed')
    ...
    try:
        ...
        print('FROZEN_LOCALIZED_EVIDENCE_PREDICTIONS_FINISHED',flush=True)
...
if __name__=='__main__':
    try: asyncio.run(run())
        ...
        print('Stopped safely: '+type(error).__name__,flush=True); raise SystemExit(1)
